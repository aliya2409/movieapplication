...
<div class="card flex-row flex-wrap">
    ...
    <div class="card-block px-2">
        <h3 class="card-title">${movie.translatedTitle}</h3>
        <c:if test="${movie.translatedTitle ne movie.originalTitle}"><p
                class="card-text">${movie.originalTitle}</p></c:if>
        <p class="card-text"><strong><fmt:message key="releaseDate"/>: </strong>${movie.releaseDate}</p>
        <p class="card-text"><strong><fmt:message key="budget"/>: </strong>$ ${movie.budget}</p>
        <p class="card-text"><strong><fmt:message key="duration"/>: </strong>${movie.duration}</p>
        <c:if test="${not empty movie.rating}"><p class="card-text"><strong><fmt:message key="rating"/>: </strong>${movie.rating}</p></c:if>
        <p class="card-text"><strong><fmt:message key="genres"/>: </strong>
            <c:forEach items="${movie.genres}" var="genre" varStatus="loop">${genre}${!loop.last ? ',' : ''}</c:forEach>
        </p>
        <p>
        ...
        <c:choose>
            <c:when test="${not empty sessionScope.user and user.roleId eq Constants.ADMIN_ROLE_ID}">
                ...
                <div class="btn-group-vertical">
                    <c:forEach items="${movie.genres}" var="genre">
                        <button onclick="location.href='${movieServlet}/deleteMovieGenre?genreId=${genre.id}'" type="button"
                                class="btn btn-danger">
                            <fmt:message key="deleteGenre"/> ${genre.name}</button>
                    </c:forEach>
                </div>
                </p>
            </c:when>
            <c:when test="${not empty sessionScope.user and user.roleId eq '1'}">
                ...
                <form action="${movieServlet}/rateMovie">
                    <input type="hidden" name="location" value="${requestScope['javax.servlet.forward.request_uri']}">
                    <input type="hidden" name="movieId" value="${movie.id}">
